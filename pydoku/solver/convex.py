...
class Convex(Solver):
    ...
    def __init__(self, *args, **kwargs):
        super(Convex, self).__init__(*args, **kwargs)

        self.p = self.clues.n**3  # size of objective
    ...
    def constraintMatrix(self):
        ...
        self.constraints = zeros((4 * n**2 + self.clues.count, n**3))
        ...
        # clues
        m = zeros((self.clues.count, self.p))

        for i, cell in enumerate(column_stack(where(self.clues.board != 0))):
            row, col = cell
    ...
    def _cvx_solve(self, verbose=False):
        if self.constraints is None:
            self.constraintMatrix()
    ...
    def check(self, verbose=False):
        ...
        if self.sol is None:
            print "Find solution..."
            self._solution(verbose=verbose)
